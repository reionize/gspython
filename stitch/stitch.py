...
def getargs():
    ...
    parser.add_argument('-t', '--stack_tmd', action='store_true', help='stacks raw heightmap data when stitching instead of normalizing for background variation')
    ...
    parser.add_argument('-f', '--output_folder', type=str, default='', help='specify location of output folder (defaults to package directory)')
...
#     print(args.object_dirs, args.allowance)
    ...
    return args.object_dirs, args.allowance, args.stack_tmd, args.ratio, args.n_bins, args.levels, args.reverse_order, args.diagnose, args.orb, args.output_folder, args.tag

# main script execution
if __name__ == '__main__':
    # read args from command line
    objects, allowance, stack_tmd, ratio, nbins, levels, reverse_order, diagnose, use_orb, out_dir, tag = getargs()
    ...
    if not os.path.exists(out+'output/'):
        print('Created /output folder')
        os.mkdir(out+'output/')
    
    # stitch scans together
    stitchscan.stitchscans(objects, allowance=allowance, normalize_height=normalize_height, ratio=ratio, nbins=nbins, levels=levels, reverse_order=reverse_order, diagnose=diagnose, mode=mode, out_dir=out_dir, tag=tag)